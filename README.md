...
## 14. GitHub automation included

This repository now includes GitHub Actions workflows for:

- `CI` (`.github/workflows/ci.yml`): YAML/JSON/Python syntax checks and Docker Compose validation.
- `ShellCheck` (`.github/workflows/shellcheck.yml`): shell script quality checks for operational scripts.
- `CodeQL` (`.github/workflows/codeql.yml`): scheduled and PR security analysis for Python code.
- `Renovate` (`.github/workflows/renovate.yml`): scheduled dependency update automation.
- `Compose Deployment Smoke Test` (`.github/workflows/compose-deployment-test.yml`): test de déploiement Docker Compose (SeaweedFS) sur runner GitHub.

### Renovate and Dependabot

To run Renovate via GitHub Actions, define this repository secret:

- `RENOVATE_TOKEN`: a GitHub token with permissions to open/update pull requests.

In addition, Dependabot is configured in `.github/dependabot.yml` for:

- GitHub Actions
- Docker (root `docker-compose.yml` and Dockerfiles)
- Python dependencies in `/beam` and `/flink`
