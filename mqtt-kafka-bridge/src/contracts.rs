...
fn required_fields() -> HashMap<&'static str, BTreeSet<&'static str>> {
    HashMap::from([
        (RAW_SENSOR_TOPIC, fields(&["device_id", "payload", "timestamp"])),
        (RAW_GPS_TOPIC, fields(&["device_id", "latitude", "longitude", "timestamp"])),
        (
            RAW_IMAGE2D_META_TOPIC,
            fields(&["bucket", "device_id", "format", "object_key", "object_uri", "timestamp"]),
        ),
        (
...
fn media_allowed_fields() -> HashMap<&'static str, BTreeSet<&'static str>> {
    HashMap::from([
        (
            RAW_IMAGE2D_META_TOPIC,
            fields(&[
                "bucket", "camera_id", "checksum", "content_type", "device_id", "format",
                "height", "ingested_at", "mqtt_topic", "object_key", "object_uri", "qos",
                "retain", "size_bytes", "source", "tags", "timestamp", "width",
            ]),
        ),
        (
            RAW_IMAGE3D_META_TOPIC,
            fields(&[
                "bucket", "capture_type", "checksum", "content_type", "coordinate_frame",
                "device_id", "format", "ingested_at", "mqtt_topic", "object_key", "object_uri",
                "point_count", "qos", "retain", "sensor_id", "size_bytes", "source", "tags",
                "timestamp",
            ]),
        ),
        (
            RAW_VIDEO2D_META_TOPIC,
            fields(&[
                "bucket", "camera_id", "checksum", "codec", "content_type", "device_id",
                "duration_ms", "format", "frame_count", "frame_rate", "height", "ingested_at",
                "mqtt_topic", "object_key", "object_uri", "qos", "retain", "size_bytes",
                "source", "tags", "timestamp", "width",
            ]),
        ),
        (
            RAW_VIDEO3D_META_TOPIC,
            fields(&[
                "bucket", "checksum", "codec", "content_type", "device_id", "duration_ms",
                "format", "frame_count", "frame_rate", "height", "ingested_at", "mqtt_topic",
                "object_key", "object_uri", "qos", "representation", "retain", "rig_id",
                "size_bytes", "source", "tags", "timestamp", "views", "width",
            ]),
        ),
    ])
}
...
fn validate_ranges(event: &Map<String, Value>, errors: &mut Vec<String>) {
    ...
    for field in ["duration_ms", "frame_count", "point_count", "size_bytes", "speed_m_s"] {
        if event.get(field).and_then(Value::as_f64).is_some_and(|value| value < 0.0) {
...
fn string_fields() -> BTreeSet<&'static str> {
    fields(&[
        "bucket", "camera_id", "capture_type", "checksum", "codec", "content_type",
        "coordinate_frame", "device_id", "format", "ingested_at", "mqtt_topic", "object_key",
        "object_uri", "representation", "rig_id", "sensor_id", "source", "timestamp",
    ])
}

fn integer_fields() -> BTreeSet<&'static str> {
    fields(&[
        "duration_ms", "frame_count", "height", "point_count", "qos", "size_bytes", "views",
        "width",
    ])
}

fn number_fields() -> BTreeSet<&'static str> {
    fields(&["altitude_m", "frame_rate", "heading_deg", "latitude", "longitude", "speed_m_s"])
}
